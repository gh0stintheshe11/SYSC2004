...
public class Room 
{
    ...
    // contains all the rooms (static field)
    ...
    /**
     * Removes and returns the given item from the room.  Returns null
     * if the item does not exist.
     * 
     * @param name The name of the item to remove
     * @return The item removed, null if not found
     */
    public Item removeItem(String name) 
    {
        // search the items for the given name
        Item item;
        Iterator<Item> iter = items.iterator();
        while (iter.hasNext()) {
            item = iter.next();
            if (item.getName().equals(name)) { // found one!
                iter.remove();
                return item;
            }
        }
    ...
    /**
     * Static method that returns all the rooms.  
     *
     * @return ArrayList<Room> the current rooms
     */
